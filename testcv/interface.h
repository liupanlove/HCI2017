...
/*
	...
	关于action的说明：(以progress为例)
		物体A按某个圆转一圈，若符合progress的圆则该物体进入可调整progress状态，
			此时返回sub_action_type为0，可以通过左右拖动调整progress，通过value(0..1)来表示希望调整到的进度
			如果想要退出则往上拖，拖到一定程度则progress条变黄，表示如果继续往上拖就会退出调整progress状态，此时返回sub_action_type为1
			如果继续往上拖超过某个阈值则退出可调整progress状态，此时返回sub_action_type为2
	press up和down不存在进入状态的说法，故sub_action_type和value都没用。
*/
struct Action {
	/*
	0 do nothing
...
public:
	...
	*/
	...
	const bool clockwise[CIRCLE_NUM] = { true, false, true, false, true, false};
	const double phase0[CIRCLE_NUM] = { .0, .0, PI, PI, PI / 2, PI / 2};
	...
	Round get_round(int type)
	{
		Round tmp;
		tmp.direction = clockwise[type];
		tmp.phase = phase0[type];
		tmp.speed = PI;
		return tmp;
	}
	/*
